...
# pdb.set_trace()
data_sorted = sorted(data, key=lambda k:k[2]) # log수로 sorting (작은게 앞)
...
# plt.plot(pre_at_N_ran, label='RANDOM', marker='o')
# # plt.plot(pre_at_N_max, label='MAX', linestyle='--')

# plt.plot(pre_at_N_geo, label='GEO', linestyle='--')
# plt.plot(pre_at_N_geo_sim, label='GEO_Euclidean', linestyle='--')
# plt.plot(pre_at_N_geo_cos, label='GEO_Cosine', linestyle='--')
# plt.plot(pre_at_N_geo_pear, label='GEO_Pearson', linestyle='--')
