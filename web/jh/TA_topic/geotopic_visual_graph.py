...
def maximizingPhi(Phi, sess, feed_dict, Phi_tensor, outputs, Z, I):
        ...
        writer = tf.summary.FileWriter("geotopic_graph", sess.graph)
    ...
    def __setTrainModule(self):
        ...
        with graph.as_default():
            ...
            with tf.variable_scope("Parameters"):
                Theta = tf.placeholder(tf.float64, shape = [N, Z], name="UserTopic")
                Phi = tf.placeholder(tf.float64, shape = [Z, I], name="TopicLocation")

            with tf.variable_scope("Inputs"):
                # Summation along users with exponentiated distances between locations
                beta_dists = tf.placeholder(tf.float64, shape = [N, I], name="ExponentiatedDistances")
            
                # Visited logs
                z_user_log = tf.placeholder(tf.int64, name="VisitedLogs")
            ...
            with tf.variable_scope("Computations"):
                ...
                Z_posterior_values = tf.placeholder(tf.float64, name="Posteriors")
            ...
            with tf.variable_scope("ProbabilityComputing"):
                # Z x 1 x I
