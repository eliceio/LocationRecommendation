...
def split_train_test(pref_final):
    ...
    for user in range(N):
        ...
        if len(visited) == 1:
            ...
            test_pref = np.random.choice(visited, size=1, replace=False)
...
def main():
    ...
    log_posterior = get_log_posterior(U, V, train, sim_u, sim_v, 0.005, 0.005, 0.05, 0.05, N, I, Z)
    ...
    while True:
        ...
        #                  x0 = V, args = (estimated_U, train, sim_u, sim_v, lambda_u, lambda_v, alpha, beta, N, I, Z),
        #                  method='Newton-CG',
        #                  options={'maxiter': 100, 'disp':True},
        #                  jac = get_grad_v)
        ...
        new_U = U - 0.1 * get_grad_u(U, V, pref_final, sim_u, sim_v, 0.005, 0.005, 0.05, 0.05, N, I, Z)
        new_V = V - 0.1 * get_grad_v(U, V, pref_final, sim_u, sim_v, 0.005, 0.005, 0.05, 0.05, N, I, Z)

        cond_u = np.sum(np.square(U - new_U)); cond_v = np.sum(np.square(V - new_V))
        cond = np.sqrt(cond_u + cond_v)
        condition = cond < 0.001
    ...
    print("time %.2f seconds are running out" % time.time() - start_time)
