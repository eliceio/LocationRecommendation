...
class TopicModel():
        ...
        def __get_visited_loc_user(self): # checked
                ...
                for index, row in df_user.iterrows():
                        usr_loc_cnt = location_counts[row["Member ID"]]
                        print("user location visited:", usr_loc_cnt)
                        if row["Member ID"] not in visited_loc_user:
                                visited_loc_user[row["Member ID"]] = [row["Restaurant ID"]]
                        else:
                                print(len(visited_loc_user[row["Member ID"]]))
                                if len(visited_loc_user[row["Member ID"]]) < (usr_loc_cnt - 1):
                                        visited_loc_user[row["Member ID"]].append(row["Restaurant ID"])
                                        print("current dictionary:", visited_loc_user[row["Member ID"]])
                                else:
                                        pass
