...
def load_data():
    ...
    # pdb.set_trace()
    df = df[['Member ID', 'Member Nickname', 'Restaurant ID', 'Restaurant Latitude', 'Restaurant Longitude', 'Restaurant Name', 'Restaurant code', 'Restaurant subcode']]
...
def idx2name(lists):
    ...
    for i, list_idx in enumerate(lists):
        loc_name.append([])
        for idx in list_idx:
            loc_name[i] += df_cut[df_cut['Restaurant ID']==uq_res_id[idx-1]]['Restaurant Name'].tolist()
